...
GitHubでソースと変更履歴を管理し、ローカルで変更をまとめた後、必要なタイミングだけCloudflare Pagesのテスト環境で画面レビューを行ってから本番配信します。本番公開は自動化せず、管理者の明示的な許可を必須とします。
...
5. 変更をGitHubの作業ブランチへコミット・プッシュして履歴を残す。コミット・プッシュだけではCloudflareへ反映しない。
6. 関連する修正とフィードバックを一つのレビュー単位にまとめる。
7. 管理者から「テスト環境へ反映」「レビュー環境を更新」などの明示依頼があった場合に限り、最新のレビュー単位をCloudflareへ1回反映する。外部連携の検証にホスト環境が不可欠な場合は、目的を説明して承認を得る。
8. レビュー環境のURL、変更ファイル、検査結果、未確定事項を管理者へ提示する。
9. 管理者のレビュー中は、本番デプロイ、`main` へのマージ、Stripe・Cloudflare本番設定の変更を行わない。
10. 管理者がレビュー環境を確認し、明示的に「デプロイして」「本番反映して」などと許可した後だけ本番反映する。
11. 許可後に内容が変わった場合は、修正をまとめてレビュー環境を1回更新し、再レビューと再許可を受ける。

## デプロイ回数の節約

- 3つのCloudflare PagesプロジェクトはGit連携を使わない手動デプロイ運用（`Git Provider: No`）とし、GitHubへのpushをデプロイの理由にしない。
- 中間確認はローカルHTTPプレビューで行う。通常の文言・レイアウト・リンク・JavaScript修正だけではCloudflare Previewを作らない。
- Cloudflare Previewは原則として1レビュー単位につき1回とし、細かな修正をまとめてから反映する。
- 同一コミットは、直前の反映失敗またはステージング設定変更がない限り再デプロイしない。
- レビューには固定のブランチ別URLを使う。個別デプロイURLを新しい確認先として乱立させない。
- Stripeサンドボックス、D1、Cloudflare Access、メールなど、ローカルでは確認できない機能だけホスト環境で検証する。
- 古いPreviewを削除しても消費済みの月間枠は戻らないため、削除を節約策として扱わない。
- 本番反映は承認済みの変更をまとめ、原則1回で完了させる。
