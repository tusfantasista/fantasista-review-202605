...
export const INTEREST_PARTICIPANT_CATEGORIES = [
  "tus_obog",
  "gakushuin_oyu",
  "tus_obog_companion",
  "gakushuin_oyu_companion",
  "eligibility_consultation",
];
export const INTEREST_ATTENDANCE_INTENTS = ["want_to_attend", "considering", "not_attending_info"];
export const INTEREST_STATUSES = [
  ...
  "duplicate",
  "invalid",
  "formal_registration_invited",
  "converted_to_formal_registration",
];
...
export async function nextInterestCode(db) {
  await db
    .prepare("INSERT OR IGNORE INTO application_sequences (name, last_value, updated_at) VALUES (?, ?, ?)")
    .bind("festa60_interest", 0, nowIso())
    .run();

  const row = await db
    .prepare("UPDATE application_sequences SET last_value = last_value + 1, updated_at = ? WHERE name = ? RETURNING last_value")
    .bind(nowIso(), "festa60_interest")
    .first();

  if (!row?.last_value) throw new Error("Failed to issue Festa 60 interest number.");
  return `INT-${String(row.last_value).padStart(6, "0")}`;
}

export async function insertInterestRegistration(db, payload, requestMeta = {}) {
  const id = newId("int");
  const interestCode = payload.interest_code || (await nextInterestCode(db));
  const now = nowIso();

  await db
    .prepare(
      `INSERT INTO festa60_interest_registrations (
        id, interest_code, last_name, first_name, last_kana, first_kana, maiden_name,
        graduation_year, graduation_year_unknown, participant_category, email, phone, attendance_intent,
        companion_status, companion_count, companion_host_category, companion_host_last_name,
        companion_host_first_name, companion_host_graduation_year, companion_host_note,
        dance_time_intent, photo_consent, volunteer_interest, donation_interest, sponsorship_interest,
        archive_material_interest, message, privacy_agreed_at, created_at, updated_at, status,
        admin_memo, user_agent, source_path, ip_hash
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .bind(
      id,
      ...
      payload.first_kana,
      payload.maiden_name || null,
      payload.graduation_year_unknown ? null : numberOrNull(payload.graduation_year),
      payload.graduation_year_unknown ? 1 : 0,
      payload.participant_category,
      String(payload.email || "").trim().toLowerCase(),
      payload.phone || null,
      payload.attendance_intent,
      payload.companion_status || null,
      numberOrNull(payload.companion_count),
      payload.companion_host_category || null,
      payload.companion_host_last_name || null,
      payload.companion_host_first_name || null,
      numberOrNull(payload.companion_host_graduation_year),
      payload.companion_host_note || null,
      payload.dance_time_intent || null,
      payload.photo_consent || "unanswered",
      payload.volunteer_interest || "unanswered",
      payload.donation_interest || "unanswered",
      payload.sponsorship_interest || "unanswered",
      payload.archive_material_interest || "unanswered",
      payload.message || null,
      now,
      now,
      now,
      ...
      requestMeta.source_path || null,
      requestMeta.ip_hash || null,
    )
    .run();

  await audit(db, {
    actor: "public_interest_form",
    action: "interest.created",
    target_type: "festa60_interest",
    target_id: id,
    details_json: JSON.stringify({
      interest_code: interestCode,
      participant_category: payload.participant_category,
      attendance_intent: payload.attendance_intent,
    }),
    user_agent: requestMeta.user_agent || null,
  });

  return getInterestRegistrationById(db, id);
}

export async function recentInterestByEmail(db, email) {
  if (!email) return null;
  return db
    .prepare(
      `SELECT id, interest_code, email, created_at
       FROM festa60_interest_registrations
       WHERE lower(email) = ?
...
export async function getInterestRegistrationById(db, idOrCode) {
  return db
    .prepare(
      `SELECT *
       FROM festa60_interest_registrations
       WHERE id = ? OR interest_code = ?
       LIMIT 1`,
    )
...
export async function listInterestRegistrations(db, filters = {}) {
  const where = [];
  const binds = [];
  if (filters.attendance_intent) {
    where.push("i.attendance_intent = ?");
    binds.push(filters.attendance_intent);
  }
  if (filters.participant_category) {
    where.push("i.participant_category = ?");
    binds.push(filters.participant_category);
  }
  if (filters.status) {
    where.push("i.status = ?");
    binds.push(filters.status);
  }
  if (filters.donation_interest) {
    where.push("i.donation_interest = ?");
    binds.push(filters.donation_interest);
  }
  if (filters.sponsorship_interest) {
    where.push("i.sponsorship_interest = ?");
    binds.push(filters.sponsorship_interest);
  }
  if (filters.archive_material_interest) {
    where.push("i.archive_material_interest = ?");
    binds.push(filters.archive_material_interest);
  }
  ...
  const result = await db
    .prepare(
      `SELECT
        ...
        (
          SELECT COUNT(*)
          FROM festa60_interest_registrations d
          WHERE lower(d.email) = lower(i.email)
        ) AS duplicate_count
       FROM festa60_interest_registrations i
...
export async function updateInterestRegistration(db, idOrCode, update, requestMeta = {}) {
  const current = await getInterestRegistrationById(db, idOrCode);
  if (!current) return null;
  const status = update.status || current.status;
  if (!INTEREST_STATUSES.includes(status)) throw new Error("Invalid interest status.");
  const adminMemo = update.admin_memo ?? update.adminMemo ?? current.admin_memo ?? null;
  const now = nowIso();

  await db
    .prepare(
      `UPDATE festa60_interest_registrations
       SET status = ?, admin_memo = ?, updated_at = ?
       WHERE id = ?`,
    )
    .bind(status, adminMemo, now, current.id)
    .run();
  ...
  await audit(db, {
    ...
    details_json: JSON.stringify({ interest_code: current.interest_code, status }),
    ...requestMeta,
  });

  return getInterestRegistrationById(db, current.id);
}
